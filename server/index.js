...
const db                               = require("./db");
const { verifyPassword, hashPassword, createToken,
        authMiddleware, requireAnalyst,
        requireAdmin }                 = require("./auth");
const { detectIocType, analyzeIoc }    = require("./analyzers/ioc");
const { scoreConfidence,
        classifySeverity }             = require("./analyzers/scoring");
const { getCampaignSummary }           = require("./analyzers/campaign");
const { generateTacticalReport,
        saveReportFile }               = require("./reports");
...
app.post("/api/auth/register", requireAdmin, (req, res) => {
  ...
  try {
    const uid = db.createUser(username, email, password, role);
    db.logAudit(req.user.sub, "register", "user", uid, { new_user: username, role });
...
app.post("/api/auth/signup", loginLimiter, (req, res) => {
  ...
  try {
    db.createUser(username, email, password, "analyst");
    const token = createToken(username, "analyst");
    db.logAudit(username, "signup");
...
app.post("/api/auth/google-signin", loginLimiter, (req, res) => {
  ...
  if (!user) {
    ...
    db.createUser(username, email, crypto.randomBytes(32).toString("hex"), "analyst");
...
app.patch("/api/auth/password", authMiddleware, (req, res) => {
  ...
  if (!current_password || !new_password) return res.status(400).json({ detail: "current_password and new_password required" });
  ...
  if (!verifyPassword(current_password, user.password_hash)) return res.status(401).json({ detail: "Current password is incorrect" });

  db.updatePassword(user.username, hashPassword(new_password));
  db.logAudit(user.username, "password_change");
...
app.get("/api/iocs", (req, res) => {
  ...
  res.json(db.listIocs(ioc_type, limit));
});

app.get("/api/iocs/search", (req, res) => {
  const { q, ioc_type, severity, source, malware_family, min_confidence, limit } = req.query;
  res.json(db.searchIocs({
    q:              q || null,
...
app.get("/api/iocs/:id", (req, res) => {
  const ioc = db.getIocById(parseInt(req.params.id));
  if (!ioc) return res.status(404).json({ detail: "IOC not found" });
  ioc.notes       = db.getIocNotes(ioc.id);
  ioc.on_watchlist = db.isOnWatchlist(ioc.id);
  res.json(ioc);
});
...
app.post("/api/iocs/analyze", requireAnalyst, async (req, res) => {
  ...
  const iocId = db.upsertIoc(value, iocType, {
  ...
  db.logAudit(req.user.sub, "analyze", "ioc", iocId, { value });
  res.json({ ...result, confidence: conf, severity, id: iocId });
});

app.post("/api/iocs/:id/notes", requireAnalyst, (req, res) => {
  const iocId   = parseInt(req.params.id);
  const content = (req.body.content || "").trim();
  if (!content) return res.status(400).json({ detail: "content is required" });
  if (!db.getIocById(iocId)) return res.status(404).json({ detail: "IOC not found" });

  const noteId = db.addIocNote(iocId, req.user.sub, content);
  db.logAudit(req.user.sub, "note", "ioc", iocId);
  res.json({ id: noteId, ioc_id: iocId, author: req.user.sub, content });
});

app.get("/api/iocs/:id/notes", (req, res) => {
  const iocId = parseInt(req.params.id);
  if (!db.getIocById(iocId)) return res.status(404).json({ detail: "IOC not found" });
  res.json(db.getIocNotes(iocId));
});
...
app.post("/api/watchlist", requireAnalyst, (req, res) => {
  const { ioc_id, reason = "", priority = "medium" } = req.body;
  if (!ioc_id) return res.status(400).json({ detail: "ioc_id required" });
  if (!db.getIocById(ioc_id)) return res.status(404).json({ detail: "IOC not found" });
  if (!["low", "medium", "high"].includes(priority))
    return res.status(400).json({ detail: "priority must be low | medium | high" });

  const entryId = db.addToWatchlist(ioc_id, req.user.sub, reason, priority);
  db.logAudit(req.user.sub, "watchlist", "ioc", ioc_id, { priority });
  res.json({ id: entryId, ioc_id, priority });
});

app.delete("/api/watchlist/:id", requireAnalyst, (req, res) => {
  db.removeFromWatchlist(parseInt(req.params.id));
  db.logAudit(req.user.sub, "watchlist_remove", "ioc", parseInt(req.params.id));
...
app.get("/api/campaigns/:id", (req, res) => {
  const summary = getCampaignSummary(parseInt(req.params.id));
...
app.post("/api/reports/:id", requireAnalyst, (req, res) => {
  const campaignId = parseInt(req.params.id);
  const tlp        = req.query.tlp || "TLP:WHITE";
  const [title, content] = generateTacticalReport(campaignId, tlp);

  if (!title) return res.status(404).json({ detail: content });

  const outPath  = saveReportFile(title, content);
  const reportId = db.saveReport(title, content, campaignId, "tactical", tlp);
  db.logAudit(req.user.sub, "report", "campaign", campaignId, { title, tlp });
  res.json({ id: reportId, title, path: outPath });
});

// ── Export ────────────────────────────────────────────────────────────────────

app.get("/api/export/stix", (req, res) => {
  const limit = Math.min(parseInt(req.query.limit) || 1000, 5000);
  const iocs  = db.listIocs(null, limit);
...
app.get("/api/audit", requireAdmin, (req, res) => {
  ...
  res.json(db.getAuditLog(limit, actor));
});
...
wss.on("connection", async (ws, req) => {
  ...
  try {
    ...
    for (const s of samples) {
      ...
      db.upsertIoc(s.sha256_hash, "hash_sha256", {
    ...
    db.logAudit("system", "collect", null, null, { total });
